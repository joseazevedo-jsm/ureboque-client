...
export const useMapScreen = () => {
  ...
  // Geocoding cache to prevent duplicate requests
  const geocodeCache = useRef(new Map());
  const geocodeTimeouts = useRef(new Map());

  const getAddressFromCoordinates = async (lat, lng) => {
    const key = `${lat.toFixed(4)},${lng.toFixed(4)}`; // Cache with reduced precision
    
    // Check cache first
    if (geocodeCache.current.has(key)) {
      const cachedAddress = geocodeCache.current.get(key);
      setMarkerCity(cachedAddress);
      return cachedAddress;
    }

    // Clear any existing timeout for this location
    if (geocodeTimeouts.current.has(key)) {
      clearTimeout(geocodeTimeouts.current.get(key));
    }
    ...
    return new Promise((resolve) => {
      const timeout = setTimeout(async () => {
        try {
          ...
          geocodeTimeouts.current.delete(key);
          resolve(address);
        } catch (error) {
      ...
      geocodeTimeouts.current.set(key, timeout);
    });
  };
  ...
  }, [userLocation, driver, hasCentered]); // More specific dependencies
  ...
  // Effect - Fetch nearby drivers with optimized polling
  useEffect(() => {
    ...
    const getPollingInterval = () => {
      if (isLoadingDrivers) return 45000; // Slower when already loading
      if (carsAround.length === 0) return 30000; // Faster when no cars found
      return 35000; // Default interval
    };
    
    // Set up interval for continuous polling with adaptive timing
    const scheduleNextPoll = () => {
      const interval = getPollingInterval();
      pollingTimerRef.current = setTimeout(() => {
        if (!service && userLocation) { // Double-check conditions
          getNearbyDrivers();
  ...
  }, [userLocation, service, isLoadingDrivers, carsAround.length]);
  ...
  const resetToInitialState = () => {
    ...
    geocodeTimeouts.current.forEach(timeout => clearTimeout(timeout));
    geocodeTimeouts.current.clear();
